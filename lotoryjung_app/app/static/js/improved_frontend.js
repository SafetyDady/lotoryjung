...
class LotoJungOrderForm {
    ...
    async validateOrder() {
        ...
        try {
            ...
            // Call improved validation API
            const response = await fetch('/api/v2/validate_order', {
                method: 'POST',
                headers: {
            ...
            if (data.success) {
                ...
                this.showAlert(data.error || 'เกิดข้อผิดพลาดในการตรวจสอบ', 'error');
            }
    ...
    async submitOrder() {
        if (!this.isValidated) {
            this.showAlert('กรุณาตรวจสอบรายการก่อนสั่งซื้อ', 'warning');
            return;
        }
...
document.addEventListener('DOMContentLoaded', function() {
    window.orderForm = new LotoJungOrderForm();
});

// Global functions for onclick handlers
function addOrderRow() {
    if (window.orderForm) {
        window.orderForm.addOrderRow();
    }
}

function validateOrder() {
    if (window.orderForm) {
        window.orderForm.validateOrder();
    }
}

function submitOrder() {
    if (window.orderForm) {
        window.orderForm.submitOrder();
    }
}

// Export for module usage
if (typeof module !== 'undefined' && module.exports) {
    module.exports = LotoJungOrderForm;
}
